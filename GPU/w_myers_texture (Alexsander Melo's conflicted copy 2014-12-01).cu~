...
__global__ void myers_kernel(const unsigned int, const unsigned int, const bitvector_64, const bitvector_64, const unsigned int, const unsigned int, boolean*, const char*, const unsigned int, const unsigned int,		 const unsigned int);

void* preprocessingPEq(bitvector_64**, const char*, const unsigned int, const unsigned int);

__device__ long advancedBlock(const bitvector_64, const bitvector_64, unsigned int, bitvector_64*, bitvector_64*, bitvector_64*,	bitvector_64*, bitvector_64*, bitvector_64*, unsigned int);



/*Global varibles*/
texture<bitvector_64, 1> UINTtexRef;
...
int main(int argc, char** argv)
{
	...
	printf("\nTotal time %lf (ms)", sw.mElapsedTime);
...
boolean* myers(const char *t, unsigned int n, const char *p, unsigned int m, unsigned int k, const unsigned int alphabet_lenght, const unsigned int device, const unsigned int threads_per_block)
{
    ...
    //bitvector_64 *d_PEq;
	//CHECK_ERROR(cudaMalloc((void**) &d_PEq, alphabet_lenght*sizeof(bitvector_64)));
    //CHECK_ERROR(cudaBindTexture(0, UINTtexRef, d_PEq, alphabet_lenght*sizeof(bitvector_64)));
	/*Copy to device*/
	...
	CHECK_ERROR(cudaMallocArray(&d_PEq, &channelDesc, alphabet_lenght, 1));
    CHECK_ERROR(cudaBindTextureToArray(UINTtexRef, d_PEq));

	/*Copy to device*/
	CHECK_ERROR(cudaMemcpyToArray(d_PEq, 0, 0, PEq, alphabet_lenght*sizeof(bitvector_64), cudaMemcpyHostToDevice));
	...
	/*========================================================================================*/
      /*size_t _free = 0,
       ...
       printf("Free memory: %f MB", ((float)_free / 1024 / 1024));
       printf("Total memory: %f MB", ((float)total / 1024 / 1024));*/
	/*========================================================================================*/
	...
	myers_kernel<<<numBlocks, threadsPerBlock>>>(slice_base, remaining, MMASK, HMASK, m, k, d_occurrence_array, d_t, n, n_slices, alphabet_lenght);
							 ...
							 const unsigned int alphabet_lenght)
{
    ...
    unsigned int reference_remaining = n_slices - remaining;
    unsigned int cooperation_remaining = (thread_reference >= reference_remaining);
    unsigned int offset = (thread_reference * slice_base) + ((thread_reference - reference_remaining)*(cooperation_remaining));
	...
	for(i = 0; i < length; i++)
	{
		...
		score += advancedBlock(MMASK, HMASK, (unsigned int) t[position], &VP, &VN, &VX, &HX, &HP, &HN, alphabet_lenght);
							...
							unsigned int alphabet_lenght)
{
	...
	EQ = tex1D(UINTtexRef, character_offset);
